 ...
 * 
 */
public class SmartChineseLunisolarCalendar {

	private static int getTarget_1(int source) {
		int target = 0;
	...
	private static int getTarget_2(int source) {
		int target = 0;
	...
	public static String toChineseLunisolarCalendar(Date dateTime) {
		int source = Integer
				.valueOf(SmartDateTime.format(dateTime, "yyyyMMdd"));
		int target = 0;
		target = getTarget_1(source);

		if (target == 0) {
			target = getTarget_2(source);
		}

		return String.valueOf(target).substring(0, 4) + "-"
				+ String.valueOf(target).substring(4, 4 + 2) + "-"
				+ String.valueOf(target).substring(6, 6 + 2);
	}
}
