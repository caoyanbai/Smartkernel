...
{
	/// <summary>
	/// Oracle（Oracle.DataAccess区分32位版本和64位版本，目前的版本是32位的，在64位操作系统上，需要同时安装32位和64位两个Client，且调用此功能的程序必须选择编译为x86）
	/// </summary>
	public static class SmartOracle
	{
		/// <summary>
		...
		public static int ExecuteNonQuery(string connectionString, string sentence, DbParameter[] parameters = null)
		{
			...
			using (DbConnection dbConnection = factory.CreateConnection())
			{
				...
				{
					//Oracle是依赖参数顺序的，通过设置BindByName可以实现按名称绑定值，否则很多反射的结果是错误的
					(dbCommand as OracleCommand).BindByName = true;
