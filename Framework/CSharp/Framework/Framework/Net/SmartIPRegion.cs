...
namespace Smartkernel.Framework.Net
{
    ...
    public class SmartIPRegion
    {
        ...
        /// 所有
        /// </summary>
        public static readonly Dictionary<int, SmartIPRegion> All = new Dictionary<int, SmartIPRegion>();

        /// <summary>
        /// 静态构造函数
        /// </summary>
        static SmartIPRegion()
        {
            using (MemoryStream memoryStream = new MemoryStream(SmartResource.IPRegion))
            {
                using (GZipStream gzipStream = new GZipStream(memoryStream, CompressionMode.Decompress))
                {
                    using (StreamReader streamReader = new StreamReader(gzipStream, Encoding.ASCII))
                    {
                        while (!streamReader.EndOfStream)
                        {
                            ...
                            var ipRegion = new SmartIPRegion()
                            {
