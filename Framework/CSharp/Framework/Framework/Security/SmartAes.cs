...
namespace Smartkernel.Framework.Security
{
    ...
    /// Aes
    /// </summary>
    public static class SmartAes
    {
        /// <summary>
        /// 加密方法
        /// </summary>
        /// <param name="input">待加密的字节数组</param>
        /// <param name="password">加密的密码（只能为16位长）</param>
        ...
        public static byte[] Encrypt(byte[] input, byte[] password, byte[] iv)
        {
            using (var aesManaged = new AesManaged())
            {
                ...
                {
                    using (var cryptoStream = new CryptoStream(memoryStream, aesManaged.CreateEncryptor(password, iv), CryptoStreamMode.Write))
                    {
        ...
        /// <param name="password">解密的密码（只能为16位长）</param>
        /// <param name="iv">偏移</param>
        /// <param name="encoding">编码</param>
        ...
        public static byte[] Decrypt(byte[] input, byte[] password, byte[] iv)
        {
            using (var aesManaged = new AesManaged())
            {
                using (var memoryStream = new MemoryStream())
                {
                    using (var cryptoStream = new CryptoStream(memoryStream, aesManaged.CreateDecryptor(password, iv), CryptoStreamMode.Write))
                    {
                        cryptoStream.Write(input, 0, input.Length);
                        cryptoStream.FlushFinalBlock();
                        return memoryStream.ToArray();
        ...
        public static Stream EncryptStream(Stream inputStream, byte[] password, byte[] iv)
        {
            ...
            using (var aesManaged = new AesManaged())
            {
                ...
                var cryptoStream = new CryptoStream(memoryStream, aesManaged.CreateEncryptor(password, iv), CryptoStreamMode.Write);
                cryptoStream.Write(datas, 0, datas.Length);
                cryptoStream.FlushFinalBlock();
                memoryStream.Position = 0;
                return memoryStream;
        ...
        public static Stream DecryptStream(Stream inputStream, byte[] password, byte[] iv)
        {
            ...
            {
                var memoryStream = new MemoryStream();
                var cryptoStream = new CryptoStream(memoryStream, aesManaged.CreateDecryptor(password, iv), CryptoStreamMode.Write);
                cryptoStream.Write(datas, 0, datas.Length);
                cryptoStream.FlushFinalBlock();
                memoryStream.Position = 0;
                return memoryStream;
